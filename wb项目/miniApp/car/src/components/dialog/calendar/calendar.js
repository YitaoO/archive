...
const conf = {
  ...
  choosePrevMonth() {
    ...
    if (typeIndex == 2) {
      ...
      const thisYear = monthsYear - 1;
      const thisMonth = new Date().getMonth() + 1;

      months.forEach(item => {
        item.disabled =
          curYear > thisYear ||
          (curYear == thisYear && thisMonth > item.value - 1)
            ? false
            : true;
        item.checked =
          monthsSelect == `${thisYear}-${item.value}` ? true : false;
      });
      this.setState({
        months: months,
  ...
  whenMulitSelect(opts = {}) {
    ...
    if (this.state.typeIndex == 1) {
      ...
      const d = new Date(
          `${thisday.year}-${
            thisday.month > 9 ? thisday.month : `0${thisday.month}`
          }-${thisday.day > 9 ? thisday.day : `0${thisday.day}`}`
        ),
        day = d.getDay(),
        date = d.getDate();
    ...
    if (this.state.typeIndex == 0) {
      if (selectedDays.length > 1) {
        ...
        for (let i = 0; i < days.length; i++) {
          if (days[i].day == selectedDays[0].day) {
            days[i].choosed = true;
          } else {
