...
var conf = {
  ...
  calculateDays: function calculateDays(year, month, curDate) {
    ...
    days.map(function (item) {
      ...
      if (_this.config.disablePastDay && timestamp - todayTimestamp < 0 && !item.disable) {
        item.disable = true;
      }
      // console.log(currYear);
      // console.log(year);

      if (currYear > year || currMonth < item.month || !!curDate && curDate < item.day) {
        item.disable = true;
      }
    });
    ...
    var tmp = {
      ...
      curYear: year,
      curMonth: month,
      nowDay: nowDay == "" ? year + "-" + month + "-" + curDate : nowDay
    };
    if (curDate) {
  ...
  choosePrevMonth: function choosePrevMonth() {
    var _state2 = this.state,
        ...
        typeIndex = _state2.typeIndex,
        months = _state2.months,
        monthsYear = _state2.monthsYear,
        monthsSelect = _state2.monthsSelect;
    ...
    if (typeIndex == 2) {
      //月选择
      var _months = this.state.months;
      var thisYear = monthsYear - 1;
      var thisMonth = new Date().getMonth() + 1;

      _months.forEach(function (item) {
        item.disabled = curYear > thisYear || curYear == thisYear && thisMonth > item.value - 1 ? false : true;
        item.checked = monthsSelect == thisYear + "-" + item.value ? true : false;
      });
      this.setState({
        months: _months,
        monthsYear: monthsYear - 1
      });
    } else {
      this.setState({
        curYear: newYear,
  ...
  chooseNextMonth: function chooseNextMonth() {
    var _state3 = this.state,
        days = _state3.days,
        curYear = _state3.curYear,
        curMonth = _state3.curMonth,
        monthsYear = _state3.monthsYear,
        months = _state3.months,
        monthsSelect = _state3.monthsSelect,
        typeIndex = _state3.typeIndex;
  ...
  whenMulitSelect: function whenMulitSelect() {
    ...
    if (this.state.typeIndex == 1) {
      ...
      selectedDays = [];
      var d = new Date(thisday.year + "-" + (thisday.month > 9 ? thisday.month : "0" + thisday.month) + "-" + (thisday.day > 9 ? thisday.day : "0" + thisday.day)),
          day = d.getDay(),
          date = d.getDate();
      ...
      days.forEach(function (item) {
        var day = item.day > 9 ? item.day : "0" + item.day;
        selectedDays.forEach(function (childItem) {
          if (day == childItem.day) {
            //TODO:这里的选中逻辑需要优化
            if (currentSelected.day < 7 || currentSelected.day > 25) {
              if (currentSelected.day < 10 && item.day < 10 || currentSelected.day > 25 && item.day > 25) {
                item.choosed = true;
              }
            } else {
              item.choosed = true;
    ...
    if (this.state.typeIndex == 0) {
      if (selectedDays.length > 1) {
        ...
        for (var _i = 0; _i < days.length; _i++) {
          if (days[_i].day == selectedDays[0].day) {
            days[_i].choosed = true;
          } else {
            days[_i].choosed = false;
          }
        }
      }
    }
  ...
  calculateNextWeekDays: function calculateNextWeekDays() {
    ...
    if (lastDayInThisMonth - lastDayInThisWeek >= 7) {
      ...
      for (var _i2 = lastDayInThisWeek + 1; _i2 <= lastDayInThisMonth; _i2++) {
        days.push({
          year: curYear,
          month: curMonth,
          day: _i2
      ...
      curYear = _Uyear;
      curMonth = _Umonth;
      for (var _i3 = 1; _i3 <= 7 - (lastDayInThisMonth - lastDayInThisWeek); _i3++) {
        days.push({
          year: curYear,
          month: curMonth,
          day: _i3
  ...
  calculatePrevWeekDays: function calculatePrevWeekDays() {
    ...
    if (firstDayInThisWeek - 7 > 0) {
      ...
      var temp = [];
      for (var _i4 = 1; _i4 < firstDayInThisWeek; _i4++) {
        temp.push({
          year: curYear,
          month: curMonth,
          day: _i4
      ...
      for (var _i5 = prevMonthDays - Math.abs(firstDayInThisWeek - 7); _i5 <= prevMonthDays; _i5++) {
        days.push({
          year: curYear,
          month: curMonth,
          day: _i5
  ...
  selectedDayWeekAllDays: function selectedDayWeekAllDays(currentDay) {
    ...
    if (firstWeekDays.find(function (item) {
      ...
      if (lastWeekDays && lastWeekDays.length < 7) {
        ...
        curYear = _Uyear3;
        curMonth = _Umonth3;
        for (var _i6 = 1, len = 7 - lastWeekDays.length; _i6 <= len; _i6++) {
          _temp.push({
            year: curYear,
            month: curMonth,
            day: _i6
